...
function generateFile (context, filePath) {
    ...
    return `
        function (__c__, __r__, __d__, __e__, __lbe__) {
                    ...
                    return i.specifiers.map(s => {
                        ...
                        if (s.exportFrom) {
                            if (s.imported === '*') {
                                ...
                                output += `;for(var __k__ in ${alias}){__k__ !== "default" && (__e__(__k__, ${alias}[__k__]))}`
                            } else {
                                let alias = context.liveBindings? `__i__["${s.local}"]` : s.local;
                                output += `;(__e__("${s.local.slice(3)}", ${alias}))`;
                            }
                        }
        ...
        `   var __nollup__import__meta__ = {
            ...
            var create_module = function (number) {
                ...
                modules[number](function (dep) {
                    ...
                    ${context.liveBindings? `
                        if (!__lbe__.hasOwnProperty(bindingName)) {
                            Object.defineProperty(__lbe__, bindingName, {
                                get: function () {
                                    return module.exports[bindingName];
                                },

                                set: function (value) {
                                    return module.exports[bindingName] = value;
                                }
                            })
                        }
                    ` : ''}
